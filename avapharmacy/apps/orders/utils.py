...
def _resolve_order_item_image(item):
    if item.product_variant and getattr(item.product_variant, 'image', None):
        return build_absolute_media_url(item.product_variant.image)
...
def send_order_confirmation_email(*, order):
    ...
    context = _build_order_email_context(
        ...
        intro=f'Hi {order.shipping_first_name or "Customer"}, we have received your order and included the summary below.',
        preheader='Order confirmation',
        primary_cta_label='Open Order',
    )
