...
- `POS_LINK_STRATEGY`
- `POS_INVENTORY_LOOKUP_URL`
- `POS_INVENTORY_LOOKUP_TOKEN`
- `POS_INVENTORY_LOOKUP_TIMEOUT_SECONDS`
- `POS_INVENTORY_LOOKUP_TTL_SECONDS`
...
## POS Inventory Lookup (Low-Stock Fallback)

When a product is low on stock (<= low_stock_threshold), the backend checks the POS
inventory API to see if another store can fulfill the order. If POS inventory is found,
the "POS Store" warehouse inventory is updated and the customer can continue ordering.

Configuration:

- `POS_INVENTORY_LOOKUP_URL`: POS inventory lookup endpoint.
- `POS_INVENTORY_LOOKUP_TOKEN`: optional bearer token for POS requests.
- `POS_INVENTORY_LOOKUP_TIMEOUT_SECONDS`: request timeout (default 8s).
- `POS_INVENTORY_LOOKUP_TTL_SECONDS`: minimum seconds between lookups for the same product (default 300s).
- `POS_LINK_STRATEGY`: how products are linked to POS items. Options:
  - `sku`: use SKU only
  - `pos_product_id` (default): require POS product ID
  - `barcode`: require barcode
  - `barcode_and_pos_id`: require barcode and POS product ID
  - `sku_or_pos_id`: accept either SKU or POS product ID
  - `sku_or_barcode`: accept either SKU or barcode
  - `any`: accept any of SKU, POS product ID, or barcode
...
{
  "items": [
    { "sku": "PAN-500", "pos_product_id": "POS-123", "barcode": "8901234567890" }
  ]
}
```
...
```json
{
  "items": [
    {
      "sku": "PAN-500",
      "pos_product_id": "POS-123",
      "quantity": 12,
      "stores": [
        { "id": "main", "name": "Downtown", "quantity": 4 },
        { "id": "west", "name": "Westlands", "quantity": 8 }
      ]
    }
  ]
}
...
- If `quantity` is missing but `stores` are provided, the backend will sum store quantities.
- The POS lookup is only attempted when stock is low or out of stock, and is throttled by TTL.
- POS inventory updates set the "POS Store" inventory for the product and mark it as POS-synced.
...
- POS inventory lookup endpoint must be reachable for low-stock checks.
